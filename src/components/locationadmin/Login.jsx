 ...
 element.className = "fa fa-eye-slash";
  document.getElementById("ibtuton").style.color = "gray"; 
   ...
   const initiateSignIn = () =>{
    ...
    let message='';
    console.log(signIn);
      if(signIn.error_password==0 && signIn.error_email==0 && signIn.error_role==0){

        Axios.post("loginCheckLocationAdmin",{email:signIn.signIn_email,password:signIn.signIn_password,admin:signIn.signIn_role,signInLocationAdmin:true})
        .then(res =>{
          console.log("request");
...
<h5 class="modal-title modal-time-title login-heading" id="exampleModalLongTitle" style={{borderBottom:'none',color:'#686868',fontFamily:'Heebo-Regular',fontWeight:'700'}}>FRMS LOGIN </h5>
...
{ errorreport  ?<span style={style}>* Role or Email or Password is Wrong</span> :''}
<form  method="post">
...
<option value="Choose your role">Choose your role</option>
  ...
  {(signIn.error_role === 1 && signIn.signIn_err===2) && <span style={{color:'red',textAlign:'left',position:'relative',display:'block',fontSize:'13px',marginTop:'4px'}}>* Select your role</span> }
  </div>
<div class="form-sec" style={{marginTop:'12px'}}>
