...
const BookingCalender = () => {
 ...
 const [facilities, setFacilities] = useState([]);
  ...
  const [paginationfac,setPaginationfac] = useState({perPage:15,totalCount:0,totalPage:1,currentPage:1});
...
const reducer3 = useSelector((state) => state.todoReducer3);
 const callPaginationfac =(s)=>{
    ...
    const callFunction=(v)=>{
      ...
      Axios.post("locationList",{perpage:15,page:1})
      .then(res =>{
        console.log("request");
        console.log(res.data);
        console.log(res.data.locationhomedeat);
    ...
    const getLocationFacility=(data)=>{
      ...
      // setLocation(data.id);
       ...
       Axios.post("getLocationFacility",{facilityid:data.id,perpage:15,page:1})
        .then(res =>{
          console.log("request");
          console.log(res.data.response);
        ...
        const clickHandler=(e)=>{
          ...
          console.log(loc);
          const regex = /\d/;
          const doesItHaveNumber = regex.test(loc);
...
if(doesItHaveNumber===true){
  ...
  lovcnew=loc;
         ...
         // return 0;
          ...
          // alert(a);
           ...
           dispatch(finalData({
            info:{month:calendernfo.month,year:calendernfo.currentYear, date:a,location:lovcnew},
            status:''
           }));
  ...
  console.log({loc:loc});
     ...
     Axios.post("get_calendar_admin",{loc:loc})
 .then(res =>{
...
console.log(calendernfo.date);
          ...
          Axios.post("get_calendar_admin",{day:day,date:calendernfo.date,loc:loc})
    .then(res =>{
      // console.log("calender");
      // console.log(res.data);
      setCalender({data:res.data.html,date:res.data.date,currentYear:res.data.currentYear,month:res.data.month});
...
if(a4.page=='/globaladmin'){
  ...
  return <Redirect to={`/globaladmin`} />;
   ...
   if (date) return <Redirect to="/BookingList" />;
...
<a onClick={()=>callFunction('location')} className={`btn-view ${callevent==='location'?'btn-view-sort':''}`} style={{textDecoration: 'none',cursor:'pointer', padding:'6px 18px'}}>SELECT LOCATION</a>
    ...
    <div class="container">     
        ...
        {callevent==='location'? <>
           ...
           <div class="col-md-6">
           { (paginationfac.currentPage==1) ? '' :   <div class="view-more-sec view-hover" style={{float: 'left', cursor: 'pointer',position:'relative'}}> 
            <a href="#"   onClick={()=>callPaginationfac('previouPage')}><i class="fa fa-angle-double-left" aria-hidden="true"></i> &nbsp; View Less
            </a>
                  </div> }
                  </div> 
                  <div class="col-md-6">
                  { (paginationfac.currentPage==paginationfac.totalPage) ? '' :  <div class="view-more-sec view-hover" style={{float: 'right', cursor: 'pointer',position:'relative'}}> 
            <a href="#"   onClick={()=>callPaginationfac('nextPage')}> View more &nbsp;<i class="fa fa-angle-double-right" aria-hidden="true"></i>
            </a>
                  </div> }
                  </div> 
                  </>
           ...
           <div  onClick={()=>callPaginationfac('previouPage')}  style={{float: 'left', cursor: 'pointer', fontSize: '46px', lineHeight: '18px', marginTop: '6px', marginLeft: '10px', color: '#f79e00',position: 'absolute',
   backgroundColor: 'rgb(145 145 145)',
    padding: '0px 6px'}}  className={`${(facilities.length<=3) && 'one-left' } ${(facilities.length<=5  && facilities.length>3) && 'two-left' }  ${(facilities.length>=6 && facilities.length>5) && 'left-three' }` }
    ><i class="fa fa-angle-left" aria-hidden="true" style={{position:'relative', left:'-1px',paddingTop:'6px',paddingBottom:'6px'}}></i>
</div>
 ...
 <div class="content-home">
   <a  target="_blank" style={{textDecoration: 'none'}}>
     ...
     <h3 class=''>#{post.location_id}</h3>
       ...
       <a onClick={()=>[setLoc(post.locationName),checkgetclalender(post.locationName)]} class="btn-view" style={{textDecoration: 'none',cursor:'pointer'}}>View</a>
     </div> 
     {/* <div class="img-sample" style={{bottom: '0px'}}><span>SAMPLE PHOTO {index}</span></div> */}
   </a>
           ...
           {/* {callevent==='location'?    <div class="col-md-12">
           <div onClick={()=>callPaginationfac('nextPage')} style={{float: 'right', cursor: 'pointer', fontSize: '46px', lineHeight: '18px', marginTop: '6px', marginLeft: '10px', color: '#f79e00',position: 'absolute',
 backgroundColor: 'rgb(145 145 145)',
    padding: '0px 6px'}}   
  className={`${(facilities.length<=3) && 'one-right' } ${(facilities.length<=5 && facilities.length>3) && 'two-right' }  ${(facilities.length>=6 && facilities.length>5) && 'right-three' }` }

    ><i class="fa fa-angle-right" aria-hidden="true" style={{position:'relative', left:'2px',paddingTop:'6px',paddingBottom:'6px'}}></i>
</div>
        ...
        {callevent==='location'? <>
        { (paginationfac.currentPage==1) ? '' :  <div class="view-more-sec view-hover" style={{position:'relative',float:'left'}} >
            <a href="#"  onClick={()=>callPaginationfac('previouPage')} style={{cursor: 'pointer'}}><i class="fa fa-angle-double-left" aria-hidden="true"></i> &nbsp; View Less
            </a>
                  </div>}
                  { (paginationfac.currentPage==paginationfac.totalPage) ? '' : <div class="view-more-sec view-hover" >
            <a href="#"  onClick={()=>callPaginationfac('nextPage')} style={{cursor: 'pointer'}}> View more &nbsp;<i class="fa fa-angle-double-right" aria-hidden="true"></i>
            </a>
                  </div>}
                  </>
...
<button type="button" class="close close-apply" data-dismiss="modal" aria-label="Close" style={{outline:'none',right:'-25px',top:'-31px'}}>
  ...
  <span aria-hidden="true" style={{outline: 'none',fontFamily: 'FontAwesome',fontSize: '33px'}}>&times;</span>
        ...
        {/* </Modal.Header> */}
          <i class="fa fa-times" aria-hidden="true" style={{float: 'right',position: 'relative',display: 'inline-block',color: '#ffffff',fontSize: '27px',left: '430px',top: '10px', cursor:'pointer'}}  onClick={()=>setShow1(false)}></i>
...
<div onClick={clickHandler} dangerouslySetInnerHTML={{__html: calendernfo.data}}></div>
  ...
  <div  class="top-title text-center" >
              <span> <i class="fa fa-circle" style={{color:'#db5656',fontSize:'13px'}} aria-hidden="true"></i> Booked</span>
              <span> <i class="fa fa-circle" style={{color:'#429f42',fontSize:'13px'}} aria-hidden="true"></i> Available</span>
          </div>
