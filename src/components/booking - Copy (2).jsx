...
console.log(location);
  ...
  const initiateReset = () =>{
    ...
    if(res.data.status==='Success'){
      ...
      setResetpassword((preValue)=>{
       ...
       const setPrintestinew =(bid)=>{
        setChShow("0");
        var elements = document.getElementsByClassName('collapse show');
        var len=elements.length;
        console.log(elements);
        for(var i = 0, length = elements.length; i < length; i++) {
          
         
          var str=String(elements[i].id);
          let result = str.includes("collapseThree");
          if(result==true){
            var newli="collapseThree"+bid;
            if(newli!=elements[i].id){
             var newid=document.getElementById(elements[i].id);
             newid.classList.remove('show');
        ...
        let b = document.getElementById("collapseThree"+bid);
    
       
      
       
        if (b.className == 'collapse show') {
       ...
       console.log(elements);
        for(var i = 0, length = elements.length; i < length; i++) {
          ...
          var str=String(elements[i].id);
          let result = str.includes("collapsefourteen");
          if(result==true){
            const last = str.charAt(str.length - 1);
            var newli="collapsefourteen"+bid;
            document.getElementById("collapsefourteenangel"+last).innerHTML =   "<i class='fa fa-angle-down fa-stack-2x'></i>";
            if(newli!=elements[i].id){
             var newid=document.getElementById(elements[i].id);
             newid.classList.remove('show');
      ...
      let b = document.getElementById("collapsefourteen"+bid);
       
        if (b.className == 'collapse show') {
          b.classList.remove('show');
          document.getElementById("collapsefourteenangel"+bid).innerHTML =   "<i class='fa fa-angle-down fa-stack-2x'></i>";
      }else{
        b.classList.add("show");
        document.getElementById("collapsefourteenangel"+bid).innerHTML =   "<i class='fa fa-angle-up fa-stack-2x'></i>";
      }
  ...
  const initiateSignIn = () =>{
    ...
    if(stat.state==='login' && applicant_id1!='0000'){
...
<div class="container">
  ...
  <div class="listing-sec">
              ...
              <Link to={`/`} class="view-button"><i class="fa fa-angle-double-left" aria-hidden="true" style={{marginRight:'4px'}}></i> 
               Back</Link>
          </div>
  </div>
  <div class="d-content d-sample-pic booking-top" >
      <div class="sample-banner-img">
         ...
         {(contentloc===true) ? 'LOCATION #'+location : location}
            </a>
        </div>
  </div>
           ...
           {   allFacilities.map((post,index)=>(          
            ...
            <div class="card-new" key={post.data.id}>
             <div class="card-header item-header"  >
               ...
               <button class="d-flex align-items-center justify-content-between button-accordion button-item" onClick={()=>setPrintestinew(post.data.id)}>
...
</button></>:<>  <button class="d-flex align-items-center justify-content-between button-accordion button-item">
...
{/* {show===post.data.id ? 
 ...
 <> 
  ...
  <input id={'t'+post.data.id}  type="checkbox"   onChange={addFacility(post.data)} class="checkboxField"  />
            ...
            <div id={'collapseThree'+post.data.id} className={`collapse ${show===post.data.id && post.amenities.length > 0 ?'show':''}`} aria-labelledby="headingThree" data-parent="#accordion">
                   ...
                   <div class="row">
                       ...
                       <div class="col-md-7" style={{paddingLeft:'0px'}}>
                           ...
                           <div class="confirm-right-sec">
                               ...
                               <h2> {(contentloc===true) ? 'LOCATION #'+location : location}
...
{   storeFacilities.map((post,index)=>(     
  ...
  <div class="card"  style={{boxShadow: 'none'}} key={post.amenitiesid} >

    <div class="card-header" id="headingfourteen">

      <h2 class="mb-0">
      {post.amenities.length > 0 ? <>  <button class="d-flex align-items-center justify-content-between button-accordion " onClick={()=>setPrintesti(post.facilityid)}  data-toggle="collapse" data-target="#collapsefourteen" aria-expanded="false" aria-controls="collapsefourteen">
...
<span>{post.facilityName}</span>  
 ...
 <span style={{visibility:'hidden'}}><p class="total-para">$18</p></span>
  ...
  <span class="fa-stack fa-sm"  id={'collapsefourteenangel'+post.facilityid}>
    ...
    <div id={'collapsefourteen'+post.facilityid} className={`collapse ${chshow===post.facilityid && post.amenities.length > 0 ?'show':''}`} aria-labelledby="headingfourteen" data-parent="#accordion">
...
<div class="col-md-12" style={{paddingLeft:'0px',paddingRight: '0px'}}>
 ...
 {(signIn.error_email === 1 && signIn.signIn_err===2) && <span style={style}>* Enter Email</span> }
  ...
  <div class="form-sec" style={{marginTop:'13px'}}>
    ...
    <div class="demo">
 <a href="#" data-tooltip="Password is your 10 digit phone number that you used to register yourself"><i class="fa fa-info-circle" aria-hidden="true" data-toggle="tooltip" data-placement="bottom"  style={{color:'red',float:'right',position:'absolute',top:'-29px',cursor:'pointer',marginLeft:'5px',right:'-163px'}}></i></a>
