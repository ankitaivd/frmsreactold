...
var perpa=val;
  ...
  setBookingInfo(res.data.bookingperpage);
    ...
    const initiateReset = () =>{
      ...
      if(res.data.status==='Success'){
       ...
       .then(res =>{
         ...
         setBookingInfo(res.data.bookingnew);
         setPaginationfac((preValue)=>{
          return { 
             perPage:preValue.perPage,
          totalCount:res.data.paginationprint.totalCount,
...
{paginationfac.perPage==15 ? 
      ...
      {   bookings.map((post,index)=>(   
       <div class="row booking-border">        
            ...
            <div class="col-md-12" style={{paddingLeft: '7px'}}>
              ...
              {(post.status==='submit' || post.status==='locassign') && <h2 style={{color:`#1053eb`}}>Submitted Pending</h2> }
              {post.status==='approved' && <h2 style={{color:'rgb(250, 139, 1)'}}>Approved</h2> }
              {post.status==='complete' && <h2 style={{color:`#48a90b`}}> Completed</h2> }
              {post.status==='estimate' && <h2 style={{color:`#48a90b`}}>Estimate Provided</h2> }
              {post.status==='rejected' && <h2 style={{color:`#ed1130`}}>Rejected</h2> }
              {post.status==='cancel' && <h2 style={{color:`#ed1130`}}>Cancelled</h2> }
              {post.status==='delete' && <h2 style={{color:`#ed1130`}}>Cancelled</h2> }
              </div>
            </div>
            ...
            <div class="col-md-8">
              <div class="booking-para">
                <p>School:- <span>
                {((post.locationName)==='Infinity') ? post.locationName :
                
                <>
                 {(isNaN(post.locationName)===false) ? '#'+post.locationName : post.locationName}
                </>
               }
 ...
 Reasons for Cancellation:- </> 
  ...
  {post.status =='cancel' ?
    ...
    </span>{post.reject_reason}</p>
   
    
    
     :'' }
{(post.approve_comment!=null && post.approve_comment!='') ? <>
<p><span  style={{fontFamily: 'Heebo-Medium',fontSize: '15px'}}>Comments:- </span>{post.approve_comment}</p>
...
:'' }
            ...
            <div class="col-md-4">
              ...
              <p style={{color:'rgb(250, 139, 1)',fontWeight:'600'}}> Booking id:- {post.bid}</p>
              {/* <p>Total Cost:- $99</p> */}
              <p>Date:- <span>{post.bookingDate}</span></p>
              <p>Time:- <span>{post.bookingStartTime+' - '+post.bookingEndTime}</span> {post.status==='draft'? 
    '' : <> 
    {post.submit_at!=null?   <span style={{marginLeft:'15px'}}   ><i id={'bookeveicon'+post.bid}  class="fa fa-plus-circle" title="Event Details"  onClick={() => {checkeventdeatials(post.bid)}} aria-hidden="true" style={{color:'#0168fa',cursor:'pointer',fontSize:'15px'}}></i></span>
  : '' }
  
  {login.address1!=null?    <span style={{marginLeft:'15px'}}   ><i id={'printveicon'+post.bid}  class="fa fa-print" title="Print"  onClick={() => {setPrintesti(post.bid)}} aria-hidden="true" style={{color:'rgb(250, 139, 1)',cursor:'pointer',fontSize:'15px'}}></i></span> 
   
  : '' }
   
    </>
    }
    {(post.status==='cancel'  || post.status==='delete')? 
    
    
    <>
    <span style={{marginLeft:'15px'}}   ><i   class="fa fa-close" title="Delete"  aria-hidden="true" data-toggle="modal" onClick={()=>setInitiate({status:'Delete',message:'You are about to delete your cancelled booking id #'+post.bid,active:true,id:post.bid})}  style={{color:'red',cursor:'pointer',fontSize:'20px'}}></i></span> 
    </> : <> 
            ...
            {(post.status==='submit' || post.status==='locassign')? 
    ...
    <div class="booking-id" id={'bookiddetails'+post.bid} style={{display:'none'}}>
    {post.submit_at!=null?  <p><span style={{fontFamily: 'Heebo-Medium',fontSize: '15px'}}>Submission Date:-</span> {post.submit_at}</p> :''}
   
    <p><span style={{fontFamily: 'Heebo-Medium',fontSize: '15px'}}>Event Name:-</span> {post.event_name}</p>
    {/* <p>Total Cost:- $99</p> */}
    <p><span style={{fontFamily: 'Heebo-Medium',fontSize: '15px'}}>Number of Participant:- </span> {post.event_participant}</p>
    <p><span  style={{fontFamily: 'Heebo-Medium',fontSize: '15px'}}>Event Description:- </span>{post.event_details}</p>
      ...
      <div className={`tab-pane fade show ${tab==='status'?'active':''}`} id="status5" role="tabpanel" aria-labelledby="status-tab5">
        ...
        <h6  class="status-heading">The status of your Booking Id <span style={{color:'rgb(250, 139, 1)',fontWeight:'600'}}>#{post.bid}</span>  </h6>
      ...
      {post.status==='draft' &&  <>
           ...
           <li >Completed</li> </>  }
           {(post.status==='submit' || post.status==='locassign') &&   <>
             <li class="active">Submitted Pending <br></br> ({post.submit_atnew})</li>
           ...
           <li class="active">Approved <br></br> ({post.approve_date})</li>
           <li >Completed</li> </> }
           {post.status==='estimate' &&
           <>
             <li class="active">Submitted Pending <br></br> ({post.submit_atnew})</li>
           
           <li class="active">Approved <br></br> ({post.approve_date})</li>
           <li class="active">Completed <br></br> ({post.complete_date})</li> </>   }
           ...
           <li class="rejectactive" >Rejected <br></br> ({post.approve_date})</li>
         </> }
         {(post.status==='cancel' && post.submit_atnew!=null) &&
           <>
             <li class="active">Submitted Pending <br></br> ({post.submit_atnew})</li>
           
           <li class="rejectactive">Cancelled <br></br> ({post.cancel_date})</li>
          </>   }
          {(post.status==='cancel' && post.submit_atnew==null) &&
           <>
             <li class="active">Draft <br></br> ({post.created_at})</li>
...
<div class="col-md-4 item-number">
  <div class="floating-label floating-new">
    ...
    <div class="select is-primary" >
      <select  value={childpost.count} id={'facnumberam'+childpost.amenities_id+'new'+childpost.school_facility_id } onChange={(event) => {increasevalue(event,childpost.school_facility_id,childpost.amenities_id)}}>
